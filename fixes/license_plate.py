...
def forbidden(plate):
    ...
    true_false=False
    for i in forbidden_combination:
        if i in plate:
            true_false=True
            return true_false
    return true_false
...
def morse(plate):
    morse_output = Morse(plate).stringToMorse()
    return morse_output.strip()

def ascii(plate):
    ascii_output = text2art(plate)
    return ascii_output

def nato(plate):
    nato ={"A": "ALPHA","B": "BRAVO","C": "CHARLIE","D": "DELTA","E": "ECHO","F": "FOXTROT","G": "GOLF","H": "HOTEL","I": "INDIA","J": "JULIET","K": "KILO","L": "LIMA","M": "MIKE","N": "NOVEMBER","O": "OSCAR","P": "PAPA","Q": "QUEBEC","R": "ROMEO","S": "SIERRA","T": "TANGO","U": "UNIFORM","V": "VICTOR","W": "WHISKEY","X": "XRAY","Y": "YANKEE","Z": "ZULU","1": "One","2": "Two","3": "Three","4": "Four","5": "Five","6": "Six","7": "Seven","8": "Eight","9": "Nine","0": "Zero"}
    nato_output = ''
    for i in plate.upper():
        if i in nato:
            word = nato[i]
            nato_output += word + ' '
